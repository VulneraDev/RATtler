...
collector heartbeat state, and dropped-event evidence.
...
Documents, and Pictures. A native FSEvents stream triggers scans within seconds,
coalesces bursts, rate-limits repeated work, and reconciles reported stream loss.
It surfaces rewrite bursts, encryption-style renames, ransom-note filenames,
mass deletion, and local canary damage without reading or uploading protected-
file contents. The 60-second scheduler remains a fallback while the native app
is running, even when its main window is closed. See the
[live file-event guide](../../docs/LIVE_FILE_EVENTS.md).
...
The app writes mode-`0600` state, operation and path-free FSEvents heartbeats,
last report, journals, baselines, and reports beneath
