...
- (void)webView:(WKWebView *)webView didFinishNavigation:(WKNavigation *)navigation {
    ...
    BOOL detectionLab = [self.viewID isEqualToString:@"detection-lab"];
    NSString *receiver = fileScan ? @"receiveFileScan" : detectionLab ? @"receiveDetectionLab" : @"receiveReport";
    NSString *payloadCall = [NSString stringWithFormat:@"window.RATtler.%@('%@');", receiver,
...
int main(int argc, const char *argv[]) {
    @autoreleasepool {
        if (argc != 4 && argc != 5) {
            fprintf(stderr, "usage: render-screenshot PAGE REPORT OUTPUT [dashboard|bluepulse|ransomware|deep-scan|detection-lab]\n");
            return 2;
        }
        ...
        if (![@[@"dashboard", @"bluepulse", @"ransomware", @"deep-scan", @"detection-lab"] containsObject:view]) {
