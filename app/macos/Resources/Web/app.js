(() => {
  ...
  function renderRansomware() {
    ...
    const recoveryActions = recoveryUpdating ? `<button id="reveal-recovery-button" class="quiet-button">Open vault</button>` : !recoveryEnabled&&!recoveryFrozen ? `<button id="enable-recovery-button" class="quiet-button primary">Enable vault</button><button id="restore-bundle-button" class="quiet-button">Restore USB copy</button>` : `${recoveryEnabled&&(severe||recoveryFrozen)?`<button id="recover-files-button" class="quiet-button primary">Recover copies</button>`:""}${recoveryEnabled?`<button id="export-bundle-button" class="quiet-button encrypted">Encrypted USB copy</button>`:""}<button id="restore-bundle-button" class="quiet-button">Restore USB copy</button><button id="reveal-recovery-button" class="quiet-button">Open vault</button>${recoveryFrozen?`<button id="resume-recovery-button" class="quiet-button warning">${recoveryEnabled?"Resume backups":"Clear freeze"}</button>`:""}`;
    $("#ransomware-content").innerHTML = `
      ...
      <article class="panel recovery-card ${recoveryFrozen||recoveryError?"frozen":recoveryEnabled||recoveryUpdating?"active":""}"><span class="recovery-symbol">${icon("shield")}</span><div><div class="status-kicker">${recoveryFrozen?"VAULT FROZEN":recoveryUpdating?"BACKGROUND REFRESH":!recoveryEnabled?"OPT-IN RECOVERY":recoveryError?"BACKUP ERROR":"VERSIONED LOCALLY"}</div><h3>${esc(recoveryHeadline)}</h3><p>${esc(recoveryCopy)}</p><small>Detection runs first · encrypted USB export · recovery never overwrites originals</small></div><div class="recovery-actions">${recoveryActions}</div></article>
    ...
    $("#export-bundle-button")?.addEventListener("click",()=>native("exportRecoveryBundle"));
    $("#restore-bundle-button")?.addEventListener("click",()=>native("restoreRecoveryBundle"));
  }
